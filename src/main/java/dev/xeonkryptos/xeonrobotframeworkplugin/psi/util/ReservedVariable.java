...
public enum ReservedVariable {
    ...
    // Scope Everywhere
    CURDIR("${CURDIR}",
           ReservedVariableScope.Global), // An absolute path to the directory where the test data file is located. This variable is case-sensitive.
    TEMPDIR("${TEMPDIR}",
            ReservedVariableScope.Global), // An absolute path to the system temporary directory. In UNIX-like systems this is typically /tmp, and in Windows c:\Documents and Settings\<user>\Local Settings\Temp.
    EXECDIR("${EXECDIR}", ReservedVariableScope.Global), // An absolute path to the directory where test execution was started from.
    ...
    CAPITAL_TRUE("${TRUE}", ReservedVariableScope.Global), // Boolean true
    CAPITAL_FALSE("${FALSE}", ReservedVariableScope.Global), // Boolean false
    ...
    SUITE_DOCUMENTATION("${SUITE_DOCUMENTATION}",
                        ReservedVariableScope.Global), // The documentation of the current test suite. Can be set dynamically using using Set Suite Documentation keyword.
    SUITE_METADATA("&{SUITE_METADATA}",
                   ReservedVariableScope.Global), // The free metadata of the current test suite. Can be set using Set Suite Metadata keyword.
    ...
    PREV_TEST_NAME("${PREV_TEST_NAME}",
                   ReservedVariableScope.TestCase), // The name of the previous test case, or an empty string if no tests have been executed yet.
    PREV_TEST_STATUS("${PREV_TEST_STATUS}",
                     ReservedVariableScope.TestCase), // The status of the previous test case: either PASS, FAIL, or an empty string when no tests have been executed.
    ...
    TEST_TAGS("@{TEST_TAGS}",
              ReservedVariableScope.TestCase), // Contains the tags of the current test case in alphabetical order. Can be modified dynamically using Set Tags and Remove Tags keywords.
    TEST_DOCUMENTATION("${TEST_DOCUMENTATION}",
                       ReservedVariableScope.TestCase), // The documentation of the current test case. Can be set dynamically using using Set Test Documentation keyword.
    // Scope Test Teardown
    ...
    @NotNull
    public final ReservedVariableScope getScope() {
        return this.scope;
    }

    @Nullable
    public final PsiElement getReferencedPsiElement(@NotNull Project project) {
        return ReservedVariableScope.getReferencedPsiElement(project);
    }
}
