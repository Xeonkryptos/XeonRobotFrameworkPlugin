...
class RobotPluginSdkFragment : SettingsEditorFragment<RobotRunConfiguration, JPanel>(
  ...
  override fun resetEditorFrom(config: RobotRunConfiguration) {
    if (currentMode == null) {
      initComponents(config.pythonRunConfiguration)
    }
    val type = if (config.pythonRunConfiguration.isUseModuleSdk) SDK_OF_MODULE else SDK_FROM_LIST
    if (currentMode != type) {
      sdkChooser.selectedItem = typeNames[type]
      (fields[SDK_OF_MODULE] as? ComboBox<*>)?.selectedItem = config.pythonRunConfiguration.module
      interpreterCombo?.reset(config.pythonRunConfiguration)
      showField(type)
    }
  ...
  override fun applyEditorTo(s: RobotRunConfiguration) {
    if (currentMode == SDK_OF_MODULE) {
      s.pythonRunConfiguration.isUseModuleSdk = true
      ...
      s.pythonRunConfiguration.isUseModuleSdk = false
      interpreterCombo?.apply(s.pythonRunConfiguration)
    }
  }
