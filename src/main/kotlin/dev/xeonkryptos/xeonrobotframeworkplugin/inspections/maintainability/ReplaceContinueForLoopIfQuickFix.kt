...
class ReplaceContinueForLoopIfQuickFix(keywordCall: RobotKeywordCall) : AbstractReplaceKeywordQuickFix(keywordCall, RobotNames.CONTINUE_FOR_LOOP_IF_NORMALIZED_KEYWORD_NAME) {
    ...
    override fun invoke(project: Project, file: PsiFile, keywordCall: RobotKeywordCall) {
        ...
        val loopControlStructure =
            RobotElementGenerator.getInstance(project).createNewConditionalStructure(conditionalContent, RobotElementGenerator.LoopControlStructureType.CONTINUE.name, emptyArray(), null)
